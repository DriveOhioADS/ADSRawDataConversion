...
search = '2023'
searchloc = os.path.join(folder_path, search + '*')
file_list = glob.glob(searchloc) #+ glob.glob(os.path.join(folder_path, search))
file_list.sort()
...
for file_path in file_list:
    print(file_path)
    filename = file_path
    delloc = filename.find(".record.")
    shortname = filename[:delloc]
    if(shortname != lastshortname and lastshortname != ""):
        print('new file set found, starting new csv')
        df.to_csv(lastshortname+xyzname)#,  index=False, header=True) #mode='a',
        gpsdf.to_csv(lastshortname+'gps.csv')
        #########
        df = pd.DataFrame(columns=['t','X','Y','Z'])
        gpsdf = pd.DataFrame()
    ...
    while reader.ReadMessage(message):
        ...
        if(message.channel_name == '/apollo/localization/pose'):
            ...
            # data_dict = {}
            # for field in msg.pose.ListFields():
...
df.to_csv(shortname+xyzname)#,  index=False, header=True) #mode='a',
gpsdf.to_csv(shortname+'gps.csv')